...
<section class="p-4 sm:p-6 antialiased">
    <div class="mx-auto max-w-screen-xl">
        ...
        <div
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.puroks.destroy', $purok->id) }}" method="POST"
                                        onsubmit="return confirm('Delete this purok?')">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            class="p-2 text-red-600 hover:bg-red-50 rounded-lg dark:hover:bg-red-900/30 transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div
                class="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4">
                <span class="text-sm text-gray-600 dark:text-gray-400">
                    Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $puroks->count() }}</span> of
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $puroks->total() }}</span> entries
                </span>
                <div>
                    {{ $puroks->links() }}
                </div>
        ...
        <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden">
            <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
                ...
                <button type="button" data-modal-hide="default-modal"
                    class="text-gray-400 hover:text-gray-900 dark:hover:text-white p-1.5 ml-auto inline-flex items-center rounded-lg">
            ...
            <form action="{{ route('admin.puroks.store') }}" method="POST" class="p-6">
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" data-modal-hide="default-modal"
                        class="px-4 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-indigo-700 focus:z-10 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Cancel</button>
                    <button type="submit"
                        class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">Create
                        Purok</button>
                </div>
            </form>
        </div>
    ...
    <div id="edit-modal" tabindex="-1" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
        <div
            class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden border dark:border-gray-700">
            <div class="p-6">
                ...
                <form id="editForm" method="POST">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <button type="submit"
                            class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Update
                            Changes</button>
                    </div>
                </form>
            </div>
